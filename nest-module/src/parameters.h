...
template <typename T>
tl::optional<T> GetParameter(const crow::query_string& query_string, const std::string& fieldname) {
...
struct MultimeterParameter {
  MultimeterParameter(const crow::query_string& query_string) {
    ...
    to_time = GetParameter<double>(query_string, "toTime").value_or(std::numeric_limits<double>::max());
