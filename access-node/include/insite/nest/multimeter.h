...
void CheckMultimeterMeasurementValid(const rapidjson::GenericObject<false, rapidjson::Value>& multimeter);

// Receives a rapidjson-Object and checks if it has all the necessary properties
// for multimeterData
void CheckMultimeterDataValid(const rapidjson::GenericObject<false, rapidjson::Value>& multimeter);
...
crow::response MultimeterByIdResponse(int api_version, uint64_t requested_multimeter_id);
...
crow::response MultimeterAttributesResponse(const crow::request& req, int api_version, int multimeter_id, const std::string& attr_name);
